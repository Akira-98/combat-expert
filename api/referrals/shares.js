...
import { createReferralShare, fetchReferralShareById } from '../_lib/referralShares.js'
...
function buildReferralUrl(req, shareId) {
  const origin = getRequestOrigin(req)
  const encodedShareId = encodeURIComponent(shareId)
  return origin ? `${origin}/?referralId=${encodedShareId}` : `/?referralId=${encodedShareId}`
}
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const result = await fetchReferralShareById({
        supabaseUrl,
        serviceRoleKey,
    ...
    return sendJson(res, 201, {
      ...result,
      referralUrl: buildReferralUrl(req, result.share.id),
    })
  } catch (error) {
    return sendServerError(res, error, 'Failed to handle referral share')
  }
}
