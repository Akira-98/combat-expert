...
export function MobileMenuSheet({
  ...
  const renderSports = (items: SportFilterItem[], mode: 'all' | 'live') =>
    items.map((sport) => {
    ...
    <div aria-modal="true" className="fixed inset-0 z-[72] xl:hidden" role="dialog">
      ...
      <aside className="ui-surface-soft absolute inset-y-0 right-0 flex w-[min(84vw,340px)] flex-col border-l p-4 shadow-2xl">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="ui-text-strong m-0 text-base font-semibold">{t('games.sports')}</p>
          </div>
        ...
        <div className="mt-5 grid gap-1.5 overflow-y-auto pr-1">
          <button className={sectionButtonClass} onClick={() => setIsLiveOpen((value) => !value)} type="button">
            <span className="flex w-7 shrink-0 items-center justify-center text-xs font-black">LV</span>
            <span className="min-w-0 flex-1 truncate">{t('games.live')}</span>
            <Chevron isOpen={isLiveOpen} />
          </button>
          {isLiveOpen ? (
            liveSports.length > 0 ? (
              <div className="grid gap-1 pl-2">{renderSports(liveSports, 'live')}</div>
            ) : (
          ...
          {isSportsOpen ? <div className="grid gap-1 pl-2">{renderSports(groupedSports.sports, 'all')}</div> : null}
          ...
          {isEsportsOpen ? (
            groupedSports.esports.length > 0 ? (
              <div className="grid gap-1 pl-2">{renderSports(groupedSports.esports, 'all')}</div>
            ) : (
