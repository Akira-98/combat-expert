export function ErrorState({
  ...
  return (
    <div className="ui-state-danger-surface rounded-lg border p-3">
...
export function EmptyState({ title, description }: { title: string; description: string }) {
  return (
    <div className="ui-surface-soft rounded-lg border border-dashed p-4 text-center">
      <p className="ui-text-strong m-0 text-sm font-semibold">{title}</p>
  ...
  return (
    <div className="grid gap-3">
      {Array.from({ length: 2 }).map((_, sectionIndex) => (
        <div key={sectionIndex} className="rounded-lg border border-slate-200 bg-white p-3">
          <div className="h-4 w-40 animate-pulse rounded bg-slate-200" />
          <div className="mt-3 grid grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-2">
            {Array.from({ length: 6 }).map((_, itemIndex) => (
              <div key={itemIndex} className="h-10 animate-pulse rounded-lg border border-slate-200 bg-slate-50" />
            ))}
          </div>
        </div>
      ))}
